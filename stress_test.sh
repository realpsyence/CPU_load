...
# Additionally for GPU utilization testing, we use glmark2:
...
# CPU+GPU Usage:
#    ./cpuload.sh [cpu load in percent] [duration in seconds] [test GPU also?]
#    ./cpuload.sh 25 10 1
...
function countdown {
   ...
   do
      CUR=$(date +%s)
      LEFT=$((END-CUR))
CORETEMP=$(sensors | grep "Core 3" | sed 's/.*:\s*+\(.*\)  .*(.*/\1/');

      printf "\r\t\tTime left: %02d:%02d:%02d\t" $((LEFT/3600)) $(( (LEFT/60)%60)) $((LEFT%60))

      printf "\rCore3 %s" $CORETEMP 

      printf "Time left: %02d:%02d:%02d\t" $((LEFT/3600)) $(( (LEFT/60)%60)) $((LEFT%60)) >> $FILE
...
# Start the script
###############################################################################
USAGE="Usage: `basename $0` [cpu load in percent] [duration in seconds] [optional 3rd arg triggers GPU test]"
...
HOST=$(hostname)
FILE="stress_test_$HOST""__$NOW.log"
STARTUP_TIME=2
...
if [ $CPULIMIT -gt $((0)) ]
then
   echo "[$(get_current_time)] => Starting stress for $CPU_LOAD_DURATION seconds."\
         | tee -a $FILE
   ...
   sudo nice -n -5 stress --cpu $NUMBER_OF_CORES --timeout $CPU_LOAD_DURATION --quiet & 
	   ...
	   # Start glmark2 if 3rd argument found.
	if [ $# -gt 2 ]
	then
	   echo "[$(get_current_time)] => Also running glmark2 for $CPU_LOAD_DURATION seconds."\
		 | tee -a $FILE

	      # START glmark2 HERE
	      #Set the nice value to obtain a higher priority than other processes
	   sudo nice -n -5 glmark2 --run-forever >> $FILE 2>&1 &
	fi
   ...
   STRESS_PIDS=$(pidof stress -o $OMIT_PID)
	...
	#Retrieve glmark2 PID
   GLMARK2_PID=$(pidof glmark2)
...
echo "[$(get_current_time)] => Killing glmark2..."		 | tee -a $FILE
sudo kill -9 $GLMARK2_PID
echo "---------------------------------------------------------" | tee -a $FILE
